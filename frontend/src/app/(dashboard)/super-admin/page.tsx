...
export default function SuperAdminDashboard() {
  ...
  const primaryStats = [
    { label: 'کل کاربران', value: (statsData?.totalUsers || 0).toLocaleString(), icon: Users, color: 'bg-blue-600', link: '/super-admin/users' },
  ...
  const entityStats = [
    { label: 'دانشجویان', value: (statsData?.students || 0).toLocaleString(), icon: GraduationCap, color: 'text-blue-600 bg-blue-50', link: '/super-admin/users?role=student' },
    { label: 'اساتید', value: statsData?.instructors || 0, icon: Briefcase, color: 'text-indigo-600 bg-indigo-50', link: '/super-admin/instructors' },
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="bg-slate-900 rounded-3xl p-8 shadow-xl flex flex-col md:flex-row justify-between items-center gap-6 relative overflow-hidden">
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-3xl font-black text-white">داشبورد کلان (Super Admin)</h1>
          </div>
        ...
        <div className="relative z-10 shrink-0 flex gap-4">
          <Link href="/super-admin/roles" className="px-6 py-4 bg-blue-600 hover:bg-blue-500 text-white rounded-2xl font-bold shadow-lg transition-all flex items-center gap-2">
            <Key className="w-5 h-5" />
            نقش‌ها و دسترسی‌ها
          </Link>
          <Link href="/super-admin/audit-logs" className="px-6 py-4 bg-slate-800 hover:bg-slate-700 text-white border border-slate-700 rounded-2xl font-bold shadow-lg transition-all flex items-center gap-2">
      ...
      <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-sm text-gray-500">بروزرسانی زنده</span>
        </div>
        ...
        {isLoadingHealth ? (
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="p-4 rounded-2xl bg-gray-50 border border-gray-100">
              <div className="text-sm text-gray-500 mb-1">وضعیت API</div>
            ...
            <div className="p-4 rounded-2xl bg-gray-50 border border-gray-100">
              <div className="text-sm text-gray-500 mb-1">دیتابیس (MongoDB)</div>
              <div className="flex items-center gap-2">
            ...
            <div className="p-4 rounded-2xl bg-gray-50 border border-gray-100">
              <div className="text-sm text-gray-500 mb-1">سرویس ذخیره‌سازی</div>
              <div className="flex items-center gap-2">
                <div className="w-2.5 h-2.5 rounded-full bg-gray-300"></div>
                <span className="font-bold text-gray-500">{healthData?.storage || 'پیکربندی نشده'}</span>
              </div>
            </div>
            <div className="p-4 rounded-2xl bg-gray-50 border border-gray-100">
              <div className="text-sm text-gray-500 mb-1">سرویس‌های خارجی</div>
              <div className="flex items-center gap-2">
                <div className="w-2.5 h-2.5 rounded-full bg-gray-300"></div>
                <span className="font-bold text-gray-500">{healthData?.externalIntegrations || 'پیکربندی نشده'}</span>
              </div>
            </div>
          </div>
        )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-white rounded-3xl p-8 shadow-sm border border-gray-100">
          <h2 className="text-xl font-bold text-gray-900 mb-6">وضعیت موجودیت‌ها</h2>
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-3xl p-8 shadow-sm border border-gray-100 h-full flex flex-col">
            <h2 className="text-xl font-bold text-gray-900 mb-6">وضعیت محتوا</h2>
            ...
            <Link href="/super-admin/courses" className="mt-4 w-full py-3 rounded-xl border border-gray-200 text-gray-700 font-medium hover:bg-gray-50 text-center transition-colors">
              مدیریت تمام دوره‌ها
            </Link>
          </div>
        </div>
