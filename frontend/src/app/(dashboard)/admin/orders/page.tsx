    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-black text-[var(--neo-text-main)]">سفارشات پلتفرم</h1>
            <p className="text-[var(--neo-text-secondary)] mt-1">مدیریت تراکنش‌ها و خریدهای دوره‌ها</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-[var(--neo-surface)] rounded-2xl shadow-sm border border-[var(--neo-border)] p-4">
        <div className="relative max-w-md">
          ...
          <input 
            ...
            className="w-full pl-4 pr-10 py-2 bg-[var(--neo-surface-2)] border border-[var(--neo-border)] rounded-xl focus:ring-2 focus:ring-amber-500 outline-none"
          />
        </div>
      </div>
      ...
      <div className="bg-[var(--neo-surface)] rounded-3xl shadow-sm border border-[var(--neo-border)] overflow-hidden">
        {isLoading ? (
          <div className="p-12 flex justify-center"><Loader2 className="w-8 h-8 animate-spin text-amber-600" /></div>
           ...
           <div className="p-12 text-center text-[var(--neo-text-secondary)] font-medium">سفارشی یافت نشد.</div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-right">
              <thead>
                <tr className="border-b border-[var(--neo-border)] bg-[var(--neo-surface-2)]/50">
                  <th className="p-4 font-bold text-[var(--neo-text-secondary)] text-sm">شناسه سفارش</th>
                  <th className="p-4 font-bold text-[var(--neo-text-secondary)] text-sm">خریدار</th>
                  <th className="p-4 font-bold text-[var(--neo-text-secondary)] text-sm">مبلغ (تومان)</th>
              ...
              <tbody>
                {filteredOrders.map((order: any) => (
                  <tr key={order._id} className="border-b border-[var(--neo-border)] hover:bg-[var(--neo-surface-2)]/50 transition-colors">
                    <td className="p-4 font-medium text-[var(--neo-text-secondary)] dir-ltr text-right text-sm">
                      #{order._id.slice(-6).toUpperCase()}
                    </td>
                    <td className="p-4 font-bold text-[var(--neo-text-main)]">
                    ...
                    <td className="p-4">
                      {order.status === 'paid' ? (
                        ...
                        <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-md text-xs font-bold bg-[var(--neo-surface-2)] text-[var(--neo-text-main)]">
