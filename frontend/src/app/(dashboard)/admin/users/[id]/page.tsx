  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex items-center gap-4">
        <button onClick={() => router.back()} className="p-2 bg-[var(--neo-surface)] rounded-xl border border-[var(--neo-border)] text-[var(--neo-text-secondary)] hover:text-[var(--neo-text-main)] transition-colors">
          <ArrowRight className="w-5 h-5" />
        </button>
        <div>
          <h1 className="text-xl font-black text-[var(--neo-text-main)]">ویرایش کاربر</h1>
          <p className="text-[var(--neo-text-secondary)] text-sm mt-1">ویرایش اطلاعات دانشجویان / اساتید / کاربران عمومی</p>
        </div>
      </div>

      <form onSubmit={handleSubmit} className="bg-[var(--neo-surface)] rounded-3xl p-6 md:p-8 shadow-sm border border-[var(--neo-border)]">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
          ...
          <div>
            <label className="block text-sm font-medium text-[var(--neo-text-main)] mb-1.5">شماره موبایل</label>
            <input type="tel" name="mobile" value={formData.mobile} onChange={handleChange} className="w-full px-4 py-2.5 rounded-xl border border-[var(--neo-border)] focus:outline-none focus:ring-2 focus:ring-[var(--neo-primary)] dir-ltr text-left" />
          </div>
          <div>
          ...
          <div>
            <label className="block text-sm font-medium text-[var(--neo-text-main)] mb-1.5">نقش کاربری <span className="text-red-500">*</span></label>
        ...
        <div className="flex justify-end pt-4 border-t border-[var(--neo-border)]">
          <button type="submit" disabled={updateMutation.isPending} className="flex items-center gap-2 px-6 py-3 bg-[var(--neo-primary)] text-white font-medium rounded-xl hover:bg-[var(--neo-primary)] transition disabled:opacity-70">
