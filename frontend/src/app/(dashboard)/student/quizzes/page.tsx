    ...
    <div className="space-y-8 pb-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-[var(--neo-text-secondary)]">ارزیابی‌ها و آزمون‌های دوره‌های ثبت‌نام شده.</p>
        </div>
      </div>

      <div className="bg-white p-4 rounded-3xl shadow-sm border border-[var(--neo-border)] flex flex-col md:flex-row items-center gap-4">
        <div className="flex-1 relative w-full">
          ...
          <input 
            ...
            className="w-full pl-4 pr-12 py-3 bg-[var(--neo-surface-2)] border border-[var(--neo-border)] rounded-xl focus:outline-none focus:ring-2 focus:ring-rose-500 focus:bg-white transition-all"
          />
        </div>
        <div className="flex items-center gap-2 w-full md:w-auto overflow-x-auto hide-scrollbar pb-2 md:pb-0">
          <button onClick={() => setFilter('all')} className={`px-4 py-2 rounded-xl font-bold whitespace-nowrap transition-colors ${filter === 'all' ? 'bg-rose-500 text-white shadow-md' : 'bg-[var(--neo-surface-2)] text-[var(--neo-text-secondary)] hover:bg-[var(--neo-surface-2)]'}`}>همه</button>
          <button onClick={() => setFilter('pending')} className={`px-4 py-2 rounded-xl font-bold whitespace-nowrap transition-colors ${filter === 'pending' ? 'bg-rose-500 text-white shadow-md' : 'bg-[var(--neo-surface-2)] text-[var(--neo-text-secondary)] hover:bg-[var(--neo-surface-2)]'}`}>آزمون‌های باز</button>
          <button onClick={() => setFilter('completed')} className={`px-4 py-2 rounded-xl font-bold whitespace-nowrap transition-colors ${filter === 'completed' ? 'bg-rose-500 text-white shadow-md' : 'bg-[var(--neo-surface-2)] text-[var(--neo-text-secondary)] hover:bg-[var(--neo-surface-2)]'}`}>انجام شده</button>
        </div>
      </div>
      ...
      {isLoading ? (
        <div className="flex flex-col items-center justify-center py-20">
          ...
          <p className="text-[var(--neo-text-secondary)]">در حال بارگذاری آزمون‌ها...</p>
        </div>
      ) : filteredQuizzes.length === 0 ? (
        <div className="bg-white rounded-3xl p-16 text-center border border-[var(--neo-border)] shadow-sm flex flex-col items-center justify-center">
          ...
          <h3 className="text-xl font-bold text-[var(--neo-text-main)] mb-2">آزمونی یافت نشد</h3>
          <p className="text-[var(--neo-text-secondary)] mb-8 max-w-sm mx-auto">شما در حال حاضر آزمونی برای انجام ندارید.</p>
        </div>
      ) : (
            ...
            <table className="w-full text-right border-collapse">
              ...
              <tbody className="divide-y divide-[var(--neo-border)]">
                {filteredQuizzes.map((item: any) => {
                  ...
                  return (
                    <tr key={quiz._id} className="hover:bg-[var(--neo-surface-2)]/50 transition-colors group">
                      ...
                      <td className="py-4 px-6">
                        <span className="inline-flex items-center gap-1 text-sm text-[var(--neo-text-secondary)]">
                           <Clock className="w-4 h-4 text-[var(--neo-text-muted)]" />
                      ...
                      <td className="py-4 px-6">
                        {isCompleted ? (
                           ...
                           <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold bg-[var(--neo-surface-2)] text-[var(--neo-text-main)]">
                             شروع نشده
                           </span>
                        )}
                      </td>
                      ...
                      <td className="py-4 px-6">
                        <Link href={`/student/quizzes/${quiz._id}`} className={`inline-flex items-center gap-1 text-sm font-bold transition ${isCompleted ? 'text-[var(--neo-text-secondary)] hover:text-[var(--neo-text-main)]' : 'text-rose-600 hover:text-rose-700'}`}>
