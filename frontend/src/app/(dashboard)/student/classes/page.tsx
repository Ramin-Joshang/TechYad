    ...
    <div className="space-y-8 pb-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-[var(--neo-text-secondary)]">مدیریت کلاس‌های آنلاین و حضوری شما.</p>
        </div>
      </div>
      ...
      {isLoading ? (
        <div className="flex flex-col items-center justify-center py-20">
          ...
          <p className="text-[var(--neo-text-secondary)]">در حال بارگذاری کلاس‌ها...</p>
        </div>
      ) : classes.length === 0 ? (
        <div className="bg-[var(--neo-surface)] rounded-3xl p-16 text-center border border-[var(--neo-border)] shadow-sm flex flex-col items-center justify-center">
          ...
          <h3 className="text-xl font-bold text-[var(--neo-text-main)] mb-2">کلاسی یافت نشد</h3>
          <p className="text-[var(--neo-text-secondary)] mb-8 max-w-sm mx-auto">شما هنوز در هیچ کلاس آنلاین یا حضوری ثبت‌نام نکرده‌اید.</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {classes.map((c: any) => {
            ...
            return (
              <div key={classInfo._id || c._id} className="bg-[var(--neo-surface)] rounded-3xl overflow-hidden border border-[var(--neo-border)] shadow-sm hover:shadow-md transition-all flex flex-col sm:flex-row">
                <div className="sm:w-48 bg-[var(--neo-surface-2)] relative">
                  ...
                  <div className="absolute top-4 right-4 bg-[var(--neo-surface)]/90 backdrop-blur text-xs font-bold px-3 py-1 rounded-full text-[var(--neo-text-main)] shadow-sm">
                    {isOnline ? 'آنلاین' : 'حضوری'}
                  </div>
                </div>
                
                <div className="p-6 flex-1 flex flex-col">
                  <h3 className="font-bold text-lg text-[var(--neo-text-main)] mb-4 line-clamp-2" title={classInfo.title}>
                    {classInfo.title}
                  </h3>
                  ...
                  <div className="mt-auto">
                    {isOnline ? (
                      ...
                      <Link href={`/classes/${classInfo.slug || classInfo._id}`} className="flex items-center justify-center gap-2 w-full bg-[var(--neo-surface-2)] text-[var(--neo-text-main)] py-2.5 rounded-xl font-bold hover:bg-[var(--neo-surface-2)] transition">
                        مشاهده اطلاعات کلاس
                      </Link>
                    )}
                  </div>
