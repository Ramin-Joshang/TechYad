...
export default function AssignmentDetailsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-20">
        ...
        <p className="text-[var(--neo-text-secondary)]">در حال بارگذاری جزئیات تکلیف...</p>
      </div>
    );
  }
  ...
  if (isError || !detailData) {
    return (
      <div className="bg-[var(--neo-surface)] rounded-3xl p-16 text-center border border-red-100 shadow-sm flex flex-col items-center justify-center">
        <h3 className="text-xl font-bold text-[var(--neo-text-main)] mb-2">تکلیف یافت نشد</h3>
  ...
  return (
    <div className="space-y-8 pb-12 animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-4xl mx-auto">
      <div>
        <Link href="/student/assignments" className="inline-flex items-center gap-2 text-sm font-bold text-[var(--neo-text-secondary)] hover:text-[var(--neo-text-main)] transition mb-6">
          <ArrowRight className="w-4 h-4" />
          بازگشت به تکالیف
        </Link>
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
          <div>
            <h1 className="text-3xl font-black text-[var(--neo-text-main)] mb-2">{assignment.title}</h1>
            <div className="flex items-center gap-4 text-sm text-[var(--neo-text-secondary)] font-medium">
              <span>دوره: <span className="text-[var(--neo-text-main)]">{assignment.courseId?.title}</span></span>
              <span>•</span>
              <span>جلسه: <span className="text-[var(--neo-text-main)]">{assignment.lessonId?.title}</span></span>
            </div>
          </div>
          
          <div className="bg-[var(--neo-surface)] px-5 py-3 rounded-2xl border border-[var(--neo-border)] shadow-sm text-center min-w-[120px]">
            <div className="text-xs text-[var(--neo-text-secondary)] mb-1">نمره کل</div>
      ...
      <div className="bg-[var(--neo-surface)] rounded-3xl p-6 md:p-8 shadow-sm border border-[var(--neo-border)] space-y-8">
        <div>
          <h2 className="text-lg font-bold text-[var(--neo-text-main)] mb-4 border-b border-[var(--neo-border)] pb-2">توضیحات تکلیف</h2>
          <div className="prose prose-sm md:prose-base max-w-none text-[var(--neo-text-main)] whitespace-pre-wrap">
        ...
        {assignment.attachments?.length > 0 && (
          <div>
            <h3 className="text-sm font-bold text-[var(--neo-text-main)] mb-3">فایل‌های ضمیمه</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {assignment.attachments.map((file: any, i: number) => (
                <div key={i} className="flex items-center justify-between p-3 bg-[var(--neo-surface-2)] border border-[var(--neo-border)] rounded-xl">
                  <div className="flex items-center gap-3 overflow-hidden">
                    <FileText className="w-5 h-5 text-[var(--neo-text-muted)] shrink-0" />
                    <span className="font-medium text-sm text-[var(--neo-text-main)] truncate">{file.name || 'فایل پیوست'}</span>
                  </div>
                  <a href={file.url || '#'} className="p-2 hover:bg-[var(--neo-border)] rounded-lg transition text-[var(--neo-text-secondary)] hover:text-[var(--neo-text-main)]">
      ...
      {submission ? (
        <div className="bg-gray-900 rounded-3xl p-6 md:p-8 shadow-sm text-white">
          <div className="flex items-center justify-between border-b border-gray-800 pb-4 mb-6">
            ...
            <div className={`px-4 py-1.5 rounded-full text-sm font-bold flex items-center gap-2 ${
              ...
              'bg-[var(--neo-primary)]/20 text-[var(--neo-primary)]'
            }`}>
          ...
          <div className="space-y-6">
            <div>
              <h3 className="text-sm font-bold text-[var(--neo-text-muted)] mb-2">متن پاسخ:</h3>
            ...
            {submission.files?.length > 0 && (
              <div>
                <h3 className="text-sm font-bold text-[var(--neo-text-muted)] mb-2">فایل‌های ارسال شده:</h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {submission.files.map((file: any, i: number) => (
                    <div key={i} className="flex items-center justify-between p-3 bg-gray-800 border border-gray-700 rounded-xl">
                      <div className="flex items-center gap-3 overflow-hidden">
                        <FileText className="w-5 h-5 text-[var(--neo-text-secondary)] shrink-0" />
                        <span className="font-medium text-sm text-gray-300 truncate">{file.name || 'فایل آپلود شده'}</span>
                      </div>
                      <a href={file.url || '#'} className="p-2 hover:bg-gray-700 rounded-lg transition text-[var(--neo-text-muted)] hover:text-white">
                        <Download className="w-4 h-4" />
                      </a>
                    </div>
                  ))}
        ...
        <div className="bg-[var(--neo-surface)] rounded-3xl p-6 md:p-8 shadow-sm border border-[var(--neo-border)]">
          <h2 className="text-xl font-bold text-[var(--neo-text-main)] mb-6 border-b border-[var(--neo-border)] pb-4">ارسال پاسخ</h2>
          
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              <label className="block text-sm font-bold text-[var(--neo-text-main)] mb-2">متن پاسخ (اختیاری اگر فایل آپلود می‌کنید)</label>
              <textarea 
                value={answerText}
                onChange={(e) => setAnswerText(e.target.value)}
                rows={6}
                className="w-full border border-[var(--neo-border)] rounded-xl p-4 focus:ring-2 focus:ring-amber-500 focus:border-amber-500 transition-all resize-y"
                placeholder="توضیحات یا پاسخ خود را اینجا بنویسید..."
              ></textarea>
            </div>

            <div>
              <label className="block text-sm font-bold text-[var(--neo-text-main)] mb-2">آپلود فایل (اختیاری)</label>
              <div className="border-2 border-dashed border-[var(--neo-border)] rounded-2xl p-8 text-center hover:bg-[var(--neo-surface-2)] transition-colors">
                {uploading ? (
                  <div className="flex flex-col items-center justify-center">
                    <Loader2 className="w-8 h-8 text-amber-500 animate-spin mb-2" />
                    <span className="text-sm text-[var(--neo-text-secondary)] font-medium">در حال آپلود...</span>
                  </div>
                ) : fileId ? (
                  ...
                  <>
                    <UploadCloud className="w-10 h-10 text-[var(--neo-text-muted)] mx-auto mb-4" />
                    <p className="text-[var(--neo-text-secondary)] font-medium mb-2">فایل خود را اینجا رها کنید یا کلیک کنید</p>
                    <p className="text-xs text-[var(--neo-text-muted)] mb-4">PDF, DOC, ZIP (حداکثر 20MB)</p>
                    <input 
                      type="file" 
                      id="file-upload" 
                      className="hidden" 
                    ...
                    />
                    <label htmlFor="file-upload" className="inline-block px-4 py-2 bg-[var(--neo-surface-2)] text-[var(--neo-text-main)] font-bold rounded-lg cursor-pointer hover:bg-[var(--neo-border)] transition">
                      انتخاب فایل
                    </label>
                  </>
                )}
