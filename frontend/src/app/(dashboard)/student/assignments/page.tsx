    ...
    <div className="space-y-8 pb-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-[var(--neo-text-secondary)]">مدیریت و ارسال تکالیف دوره‌های آموزشی.</p>
        </div>
      </div>

      <div className="bg-[var(--neo-surface)] p-4 rounded-3xl shadow-sm border border-[var(--neo-border)] flex flex-col md:flex-row items-center gap-4">
        <div className="flex-1 relative w-full">
          ...
          <input 
            ...
            className="w-full pl-4 pr-12 py-3 bg-[var(--neo-surface-2)] border border-[var(--neo-border)] rounded-xl focus:outline-none focus:ring-2 focus:ring-amber-500 focus:bg-[var(--neo-surface)] transition-all"
          />
        </div>
        <div className="flex items-center gap-2 w-full md:w-auto overflow-x-auto hide-scrollbar pb-2 md:pb-0">
          <button onClick={() => setFilter('all')} className={`px-4 py-2 rounded-xl font-bold whitespace-nowrap transition-colors ${filter === 'all' ? 'bg-amber-500 text-white shadow-md' : 'bg-[var(--neo-surface-2)] text-[var(--neo-text-secondary)] hover:bg-[var(--neo-surface-2)]'}`}>همه</button>
          <button onClick={() => setFilter('pending')} className={`px-4 py-2 rounded-xl font-bold whitespace-nowrap transition-colors ${filter === 'pending' ? 'bg-amber-500 text-white shadow-md' : 'bg-[var(--neo-surface-2)] text-[var(--neo-text-secondary)] hover:bg-[var(--neo-surface-2)]'}`}>در انتظار انجام</button>
          <button onClick={() => setFilter('submitted')} className={`px-4 py-2 rounded-xl font-bold whitespace-nowrap transition-colors ${filter === 'submitted' ? 'bg-amber-500 text-white shadow-md' : 'bg-[var(--neo-surface-2)] text-[var(--neo-text-secondary)] hover:bg-[var(--neo-surface-2)]'}`}>ارسال شده</button>
          <button onClick={() => setFilter('graded')} className={`px-4 py-2 rounded-xl font-bold whitespace-nowrap transition-colors ${filter === 'graded' ? 'bg-amber-500 text-white shadow-md' : 'bg-[var(--neo-surface-2)] text-[var(--neo-text-secondary)] hover:bg-[var(--neo-surface-2)]'}`}>تصحیح شده</button>
        </div>
      </div>
      ...
      {isLoading ? (
        <div className="flex flex-col items-center justify-center py-20">
          ...
          <p className="text-[var(--neo-text-secondary)]">در حال بارگذاری تکالیف...</p>
        </div>
        ...
        <div className="bg-[var(--neo-surface)] rounded-3xl p-16 text-center border border-[var(--neo-border)] shadow-sm flex flex-col items-center justify-center">
          ...
          <h3 className="text-xl font-bold text-[var(--neo-text-main)] mb-2">تکلیفی یافت نشد</h3>
          <p className="text-[var(--neo-text-secondary)] mb-8 max-w-sm mx-auto">برای این وضعیت یا جستجو تکلیفی وجود ندارد.</p>
        </div>
      ) : (
            ...
            <table className="w-full text-right border-collapse">
              <thead>
                <tr className="bg-[var(--neo-surface-2)] border-b border-[var(--neo-border)]">
                  ...
                  <th className="py-4 px-6 font-bold text-[var(--neo-text-main)] text-sm">عملیات</th>
                </tr>
              </thead>
                ...
                {filteredAssignments.map((item: any) => {
                  ...
                  return (
                    <tr key={assignment._id} className="hover:bg-[var(--neo-surface-2)]/50 transition-colors group">
                      ...
                      <td className="py-4 px-6">
                        {assignment.deadline ? (
                          <span className={`text-sm ${isLate ? 'text-red-500 font-bold' : 'text-[var(--neo-text-secondary)]'}`}>
                            {new Date(assignment.deadline).toLocaleDateString('fa-IR')}
                          </span>
                        ) : (
                          <span className="text-[var(--neo-text-muted)] text-sm">بدون مهلت</span>
                        )}
                      </td>
                      <td className="py-4 px-6">
                        {!submission ? (
                           <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold ${isLate ? 'bg-red-100 text-red-700' : 'bg-[var(--neo-surface-2)] text-[var(--neo-text-main)]'}`}>
                          ...
                          <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold bg-blue-100 text-[var(--neo-primary)]">
                      ...
                      <td className="py-4 px-6 font-bold">
                        {submission?.status === 'graded' ? (
                          <span className="text-[var(--neo-text-main)]">{submission.score} <span className="text-xs text-[var(--neo-text-muted)] font-normal">/ {assignment.maxScore}</span></span>
                        ) : (
